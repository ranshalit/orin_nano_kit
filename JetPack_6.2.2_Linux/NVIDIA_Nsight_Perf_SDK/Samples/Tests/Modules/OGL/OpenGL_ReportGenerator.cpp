...
namespace nv { namespace perf { namespace test {
    ...
    NVPW_TEST_CASE("ReportGenerator")
    {
        ...
        if (!pWindow)
        {
            NVPW_TEST_MESSAGE("glfwCreateWindow() failed, test is skipped.\n");
            return;
        }
        ...
        GLenum err = glewInit();
        NVPW_REQUIRE_EQ(err, GLEW_OK);

        if (!OpenGLIsNvidiaDevice())
        {
            NVPW_TEST_MESSAGE(OpenGLGetDeviceName().c_str(), " is not an NVIDIA Device, test is skipped.\n");
            return;
        }
        if (!profiler::OpenGLIsGpuSupported())
        {
            NVPW_TEST_MESSAGE(OpenGLGetDeviceName().c_str(), " is unsupported, test is skipped.");
            return;
        }

        profiler::ReportGeneratorOpenGL nvperf;
        ...
        {
#ifdef WIN32
            nvperf.outputOptions.directoryName = "HtmlReports\\TestOpenGL\\ReportGenerator_EmptyFrame";
#else
            nvperf.outputOptions.directoryName = "HtmlReports/TestOpenGL/ReportGenerator_EmptyFrame";
#endif
